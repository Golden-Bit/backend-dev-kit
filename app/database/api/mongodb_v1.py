...
def get_current_user(token: str) -> Any:
    try:
        ...
        # Questo è il dict che Cognito ti restituisce (contenente Username e UserAttributes)
        user = response.json()

        # 1) Trovo l'attributo custom:databases
        user_attributes = user.get("UserAttributes", [])
        databases = None
        for attr in user_attributes:

            if attr["Name"] == "custom:databases":
                databases = attr["Value"]

                break

        # 2) Se esiste, converto la stringa JSON in list/dict Python
        if databases:
            try:
                user["databases"] = databases
            except json.JSONDecodeError:
                # Se per qualche motivo non è un JSON valido, fallback a lista vuota
                user["databases"] = []
        else:
            # Se non c'è proprio, vuol dire che l'utente non ha ancora l'attributo
            user["databases"] = []
...
def update_user_databases(token: str, databases: List[Dict[str, Any]]):
    ...
    # { "access_token": token, "attributes": [ {"Name": "custom:databases", "Value": <JSON string>} ] }
        ...
        payload = {
            "access_token": token,
            "custom_attributes": {
                "custom:databases": json.dumps(databases)
            }
        }

        update_resp = requests.post(f"{AUTH_SERVICE_URL}/update-custom-attributes", json=payload)
...
@router.post("/{db_name}/test_api/{collection_name}/", summary="Recupera tutti i documenti di una collezione",
             response_description="Elenco dei documenti nella collezione")
async def test_api(db_name: str, collection_name: str, filter: Optional[Dict[str, Any]] = None, token: str = ""):
    """
    Recupera tutti i documenti di una collezione specifica, con la possibilità di applicare un filtro.
    """


    return "REQEUST COMPLETED"
