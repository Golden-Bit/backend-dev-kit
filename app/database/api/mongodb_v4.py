...
class DatabaseCreationRequest(BaseModel):
    db_name: str
    # Optional: specifica se i permessi del DB sono globali oppure custom.
    # Default "global" significa che l'utente ha un'unica regola che si applica a tutte le collezioni.
    permission_type: Optional[str] = "global"  # Valori possibili: "global", "custom"
...
def verify_user_database(db_name: str, user: Dict[str, Any], required_permission: str = "read"):
    """
    Verifica che il database esista nei metadata dell'utente e, se il tipo di permesso è "global",
    controlla che l'utente abbia il permesso richiesto.
    Se il tipo è "custom", non viene applicato il controllo globale.
    """
    db_record = find_database_record(db_name, user)
    if not db_record:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND,
                            detail="Database non trovato nei metadata dell'utente.")
    if db_record.get("permission_type", "global") == "global":
        if not check_db_permission(db_record, user.get("username"), required_permission):
            raise HTTPException(status_code=status.HTTP_403_FORBIDDEN,
                                detail=f"Non sei autorizzato ad eseguire operazioni con permesso '{required_permission}' sul database.")
    # In caso di "custom", i permessi saranno gestiti a livello di collezione.
...
def find_collection_record(db_record: Dict[str, Any], collection_name: str) -> Optional[Dict[str, Any]]:
    """
    Cerca il record relativo alla collezione nel record del database.
    """
...
def verify_user_collection(db_name: str, collection_name: str, user: Dict[str, Any], required_permission: str = "read"):
    ...
    Se il database ha permessi globali, questi prevalgono; altrimenti, si controlla il permesso specifico della collezione.
    """
    db_record = find_database_record(db_name, user)
    if not db_record:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND,
    ...
    if db_record.get("permission_type", "global") == "global":
        ...
        # Se il tipo è "custom", controlla il permesso a livello di collezione
...
async def create_user_database(request: DatabaseCreationRequest, token: str):
    ...
    I metadata del database includono:
      - db_name, host, port
      - owner: username
      - permission_type: "global" (default) oppure "custom"
    ...
    try:
        ...
        if not any(db.get("db_name") == prefixed_db_name for db in user_databases):
            new_db = {
                ...
                "port": port,
                "owner": username,
                "permission_type": request.permission_type,  # "global" o "custom"
                "shared_with": [],
                "collections": []
            }
...
async def create_collection(db_name: str, collection_name: str, token: str):
    ...
    Se il database ha permessi globali, si verifica a livello di DB; se custom, il controllo è a livello di collezione.
    Dopo la creazione, la collezione viene aggiunta ai metadata del database con:
      - collection_name
      - owner: lo stesso del database
      - shared_with: lista vuota
    """
    current_user = get_current_user(token)
    # Verifica a livello di database: se il tipo è "global", controlla globalmente; se "custom", saltare il controllo globale.
    ...
    if db_record.get("permission_type", "global") == "global":
        if not check_db_permission(db_record, current_user.get("username"), "write"):
            raise HTTPException(status_code=status.HTTP_403_FORBIDDEN,
                                detail="Non sei autorizzato a creare collezioni in questo database.")
    # Se il database è custom, non esiste un permesso globale e si gestirà a livello di collezione
    try:
        ...
        if not any(coll.get("collection_name") == collection_name for coll in collections):
            new_coll = {"collection_name": collection_name, "owner": db_record.get("owner"), "shared_with": []}
...
async def delete_collection(db_name: str, collection_name: str, token: str):
    ...
    Richiede il permesso "write" sul database (global) oppure sul livello di collezione (custom).
    Dopo l'eliminazione, aggiorna i metadata rimuovendo il record della collezione.
    """
    current_user = get_current_user(token)
    # Se il database è global, verifica a livello di DB; altrimenti, verifica a livello di collezione
    db_record = find_database_record(db_name, current_user)
    if not db_record:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Database non trovato nei metadata.")
    if db_record.get("permission_type", "global") == "global":
        if not check_db_permission(db_record, current_user.get("username"), "write"):
            raise HTTPException(status_code=status.HTTP_403_FORBIDDEN,
                                detail="Non sei autorizzato a eliminare collezioni in questo database.")
    else:
        verify_user_collection(db_name, collection_name, current_user, required_permission="write")
    try:
...
async def add_item(db_name: str, collection_name: str, data: Dict[str, Any], token: str):
    ...
    Per operazioni su documenti in collezioni, se il database ha permessi globali, si verifica a livello di DB;
    se custom, si richiede che l'utente abbia il permesso specifico sulla collezione.
    """
    current_user = get_current_user(token)
    db_record = find_database_record(db_name, current_user)
    if not db_record:
    ...
    if db_record.get("permission_type", "global") == "global":
        if not check_db_permission(db_record, current_user.get("username"), "write"):
            raise HTTPException(status_code=status.HTTP_403_FORBIDDEN,
                                detail="Non sei autorizzato a eseguire operazioni di scrittura in questo database.")
    else:
        verify_user_collection(db_name, collection_name, current_user, required_permission="write")
    try:
        response = requests.post(f"{MONGO_SERVICE_URL}/{db_name}/{collection_name}/add_item/", json=data)
        if response.status_code != 200:
            raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST,
...
async def get_items(db_name: str, collection_name: str, filter: Optional[Dict[str, Any]] = None, token: str = ""):
    ...
    Per operazioni di lettura, se il database ha permessi globali, si verifica a livello di DB;
    se custom, si richiede il permesso specifico sulla collezione.
    """
    current_user = get_current_user(token)
    db_record = find_database_record(db_name, current_user)
    if not db_record:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Database non trovato nei metadata.")
    if db_record.get("permission_type", "global") == "global":
        if not check_db_permission(db_record, current_user.get("username"), "read"):
            raise HTTPException(status_code=status.HTTP_403_FORBIDDEN,
                                detail="Non sei autorizzato a leggere in questo database.")
    else:
        verify_user_collection(db_name, collection_name, current_user, required_permission="read")
...
async def update_item(db_name: str, collection_name: str, item_id: str, item: Dict[str, Any], token: str = ""):
    ...
    Per operazioni di scrittura, se il database ha permessi globali, si verifica a livello di DB;
    se custom, si richiede il permesso specifico sulla collezione.
    """
    current_user = get_current_user(token)
    db_record = find_database_record(db_name, current_user)
    ...
    if db_record.get("permission_type", "global") == "global":
        if not check_db_permission(db_record, current_user.get("username"), "write"):
            raise HTTPException(status_code=status.HTTP_403_FORBIDDEN,
                                detail="Non sei autorizzato a scrivere in questo database.")
    else:
        verify_user_collection(db_name, collection_name, current_user, required_permission="write")
    try:
...
async def delete_item(db_name: str, collection_name: str, item_id: str, token: str = ""):
    ...
    Per operazioni di eliminazione, se il database ha permessi globali, si verifica a livello di DB;
    se custom, si richiede il permesso specifico sulla collezione.
    """
    current_user = get_current_user(token)
    db_record = find_database_record(db_name, current_user)
    ...
    if db_record.get("permission_type", "global") == "global":
        if not check_db_permission(db_record, current_user.get("username"), "write"):
            raise HTTPException(status_code=status.HTTP_403_FORBIDDEN,
                                detail="Non sei autorizzato a eliminare documenti in questo database.")
    else:
        verify_user_collection(db_name, collection_name, current_user, required_permission="write")
    try:
...
async def check_db_permission_endpoint(db_name: str, required_permission: str, token: str):
    """
    Verifica se l'utente possiede il permesso specificato sul database.
    """
    current_user = get_current_user(token)
    db_record = find_database_record(db_name, current_user)
    if not db_record:
...
async def grant_collection_permission(db_name: str, collection_name: str, permission_req: PermissionGrantRequest, token: str):
    ...
    # Aggiorna il record della collezione nel database
...
async def check_collection_permission_endpoint(db_name: str, collection_name: str, required_permission: str, token: str):
    """
    Verifica se l'utente possiede il permesso specificato sulla collezione.
    """
    current_user = get_current_user(token)
    db_record = find_database_record(db_name, current_user)
    if not db_record:
